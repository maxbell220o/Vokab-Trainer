# Vokab-Trainer
Ist ein eifacher Vokabeltrainer. Nicht Schön, aber Gut.

## Instalier Anleitung

1. Herunterladen Des Repos
2. Produckt-Key anfragen (boeden.versorgung.6f@icloud.com) (Code ID: JANUAR)
3. Program starten und code eingeben.

## Weitere Infos:
- Volständig Kostenlos (Produckt Key Prüft nur das das Produkt nicht in Falsche Hände gerät) 
- Die App ist kein Auslauf Produkt.
- Bei Beschwerden oder Verbesserungsvorschlägen melden sie sich unter boeden.versorgung.6f@icloud.com.


## Verion 1:

### Funtionen:
- Merere Fächer(Englisch,Latein)
- Fachsystem 1-5
- Widerhole der Falschen Vokabeln

### Bilder:
<img width="1600" height="861" alt="Screenshot 2026-02-07 124738" src="https://github.com/user-attachments/assets/aba4aa86-ab30-4bae-aa9f-d9fcc709a1cf" />
Main Window
<img width="1600" height="860" alt="Screenshot 2026-02-08 081048" src="https://github.com/user-attachments/assets/244f63d7-65b7-4e35-ac85-260d1b25a90b" />
Trainer





https://github.com/user-attachments/assets/ddf6e8f7-58ad-4f83-a341-8237587111ae

See it in Action


## Schulen:

Für Schulen bieten wir Sonderlizenzen an.
Natürlich kostenlos.
schreiben sie uns (boeden.versorgung.6f@icloud.com).
